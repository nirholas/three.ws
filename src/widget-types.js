...
const TYPE_SCHEMAS = {
	...
	'passport': brandSchema.extend({
		...
		agentId:              z.string().regex(/^\d+$/, 'agentId must be a uint256 string').nullable().default(null),
		wallet:               z.string().regex(/^0x[0-9a-fA-F]{40}$/).nullable().default(null),
...
export function validateConfig(type, config) {
	...
	if (!res.success) {
		const err = new Error(res.error.issues.map((i) => `${i.path.join('.') || 'config'}: ${i.message}`).join('; '));
