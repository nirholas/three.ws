...
- **Mitigation:** ReputationRegistry already enforces one score per (reviewer, agent). Beyond that, the recommended reputation surface for v1 is *measured* signals from validators (render success, load p95, A2A handshake), not user thumbs-up. See [3D_AGENT_CARD.md](3D_AGENT_CARD.md) §"Reputation".
