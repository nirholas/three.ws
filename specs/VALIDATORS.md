...
A validation report says "I checked agent #N's model and it is sound." For that statement to carry weight, consumers need to know *who* signed it. Open writes would let any address claim "verified" status, collapsing the trust signal to zero. Allow-listing keeps the set small, named, and accountable.
