...
## Target audience
Developers and product builders discovering 3D Agent for the first time. They may have seen the landing page or a widget embed and want to understand what the platform is, what problems it solves, and whether it's the right tool for them.
...
- **ERC-8004** — the on-chain standard for registering 3D agent identity
...
- vs. plain `<model-viewer>` (Google) — 3D Agent adds AI runtime, skills, memory, identity
- vs. general LLM chatbots — 3D Agent adds embodiment (avatar), 3D scene control, glTF understanding
- vs. Unity/Unreal WebGL exports — 3D Agent is lightweight (no game engine runtime), browser-native, open
