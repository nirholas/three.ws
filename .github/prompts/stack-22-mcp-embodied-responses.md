...
[api/mcp.js](api/mcp.js) currently exposes CRUD tools (list/get/validate). To move up the stack, the MCP server should return tool results that *contain* an embodied agent render — so Claude.ai, Cursor, and other MCP clients can display the agent, not just text.
...
- `summon_agent` — args: `{ slug, skill?, input? }` → returns:
  ```json
  {
    "content": [
      ...
      { "type": "resource", "resource": { "uri": "https://3dagent.vercel.app/agent/satoshi?kiosk=1&skill=greet", "mimeType": "text/html" } }
    ]
  }
  ```
