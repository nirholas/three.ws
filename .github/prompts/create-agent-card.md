...
Both `features.html` (line 90) and `public/.well-known/agent-registration.json` reference `https://three.ws/.well-known/agent-card.json` as the A2A discovery endpoint, but **no such file exists**. This is a dead link and breaks the A2A (Agent-to-Agent) protocol integration.
...
- `curl https://three.ws/.well-known/agent-card.json` returns valid JSON
