...
async function handleDetail(req, res, id) {
	...
	const priceRows = await sql`
		SELECT skill, currency_mint, amount 
		FROM agent_skill_prices 
		WHERE skill_id IN (SELECT id FROM skills WHERE agent_id = ${id})
	`;
...
async function handleDetail(req, res, id) {
	...
	const [priceRows, purchasedRows] = await Promise.all([
		sql`SELECT skill_name, currency_mint, amount FROM agent_skill_prices WHERE agent_id = ${id}`,
		auth ? sql`SELECT skill_name FROM skill_purchases WHERE user_id = ${auth.userId} AND agent_id = ${id}` : Promise.resolve([]),
	]);
	...
	const skill_prices = Object.fromEntries(
		priceRows.map((p) => [p.skill_name, { amount: p.amount, currency_mint: p.currency_mint }]),
	);
