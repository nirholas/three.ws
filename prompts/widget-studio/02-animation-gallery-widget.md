...
This is the widget that makes Mixamo-style rigs feel like _a product_. A rigged avatar with seven animations should ship as an interactive showcase in one click from Studio.
...
if (window.VIEWER.widget?.type === 'animation-gallery') {
	...
	this.view(resolvedModel, '', new Map()).then(() => {
		...
		this.viewer.whenAnimationsReady().then(() => {
			this._widgetRuntime = mountAnimationGallery(
				this.viewer,
				window.VIEWER.widget.config,
				this.el,
			);
		});
	});
