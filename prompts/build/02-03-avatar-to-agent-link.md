...
**Depends on:** 02-01
**Blocks:** 03-_ (edit), 04-_ (embed assumes every avatar has an agent)

## Why it matters

Today the database has `avatars` and `agent_identities` but nothing binds them on creation. A user finishes the selfie flow and has a 3D mesh — but no _agent_, which is the actual product. This prompt makes "avatar" and "agent" inseparable from the first save.
