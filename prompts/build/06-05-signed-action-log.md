...
The onchain identity is worth nothing if the agent's _actions_ are unsigned — a host can't distinguish real moves from forged ones. Signing each action with the owner wallet (off-chain, stored in `agent_actions.signature`) makes the action log cryptographically attributable.
...
3. **Unsigned fallback** — anonymous visitors (no linked wallet) continue to write actions _without_ signatures; those rows get `signature IS NULL`. Signed vs unsigned is filterable.
