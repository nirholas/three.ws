...
- Target tabs: `iframe`, `web-component`, `claude-artifact`, `chat-plugin`, `link`.
...
| `prompts/build/05-*.md`                                  | Drives the claude-artifact + chat-plugin snippet strings once that pillar ships. It's OK if those hosts' runtimes don't exist yet — the snippet picker still generates valid strings for future use. |
...
| `chat-plugin`  | JSON plugin manifest snippet (from `05-03`). Same degrade-gracefully fallback.                                                                                                                                                                                  |
...
- [ ] Share panel shows 5 target tabs (claude-artifact, chat-plugin may be placeholders).
