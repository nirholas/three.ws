...
| 05 | R2 bucket CORS (retired 2026-09-10, order deleted) | **DONE, and no credential was ever minted** | The `S3_*` pair resolved off the Cloud Run service is admin-scoped, so `Get`/`PutBucketCors` both work with it. Canonical split policy applied: `public-read` world-open for GET/HEAD, `browser-upload` origin-locked for PUT. `node scripts/set-r2-cors.mjs --probe` exits 0 with all nine origins matching. Every prior pass filed this as owner-gated on the finding that the only reachable token was object-scoped; that finding was wrong |
...
| 08 | [OKX chat bot: move off the codespace](../907-backlog-08-okx-chat-bot-always-on.md) | **deployed 2026-09-04** and still beating (re-measured 2026-09-09 from `bot_heartbeat`, no `gcloud` needed); only the reply lane is blocked, and the AI-lane chain committed the same day has not shipped. `npm run okx:bot` now refuses to start a rival daemon while that host is beating (2026-09-09, after the order's own first instruction started one) | one (re-measured 2026-09-24): the API half is live (`c8f10f437` serves the agent-scoped proxy path), so what is left is installing and authenticating `gcloud` and running `npm run okx:bot:deploy`, which provisions the free gateway reply lane and ships the lease-aware worker. Clearing the GCP billing hold is optional now: it moves the bot back to Claude on Vertex by itself |
