...
| [pumpfun-trading.md](../917-roadmap-pumpfun-trading.md), [pumpfun-trading-arena.md](../918-roadmap-pumpfun-trading-arena.md) | Two overlapping strategy documents for the trading product. They are plans, not work orders. The wedge delta was retired on 2026-09-03 (readable in git history) once its inventory was checked against the repo: the wedge itself (`/claim-wallet`), the Trader Passport, the meta-allocator, fade-the-influencer (`/ghost-copy`), the Clip Director and the 3D trading floor had all shipped, and the last unshipped, non-owner-gated mechanic (alpha-drip, tiered release of a leader's signal) shipped with it. What the wedge left open is owner-gated, not unbuilt: a per-copy execution fee is a pricing decision, and a house-bankrolled first trade spends real money per new user. Its §6 consolidation (fold the arena draft into the master plan) stays open on purpose: both drafts predate the em-dash ban, so folding them verbatim would add hundreds of lines that `npm run check:rules` rejects, and rewriting a whole plan's prose to move it is the catastrophic diff that rule exists to avoid. Fold it when one of the two is being rewritten anyway. Re-verified 2026-09-09: everything the arena plan describes is built and its migrations are applied in production, so the arena file stays on disk for one reason only, the owner-gated deploy of the `agent-sniper` worker, whose running image predates a month of committed worker code (including the graduated-position AMM exits the plan names as its blocker) and therefore also predates the Risk Officer that needs production shadow rows before anyone arms it. Tracked as row 19 in [production-100-OWNER-ACTIONS.md](production-100-OWNER-ACTIONS.md). The trading plan's own inventory was re-derived the same day: sections 3, 4, 5, 13 and 15 are live (the Coin Intelligence engine is `pump_coin_intel` / `pump_coin_wallets` / `pump_coin_outcomes` / `pump_intel_weights`, with its learned weights already read by `workers/agent-sniper/scorer.js`), and of the section 12 plays only 12.8 was both unbuilt and unblocked. It shipped as the Fade Radar (`/fade`, `api/_lib/fade-radar.js`, `docs/fade-radar.md`): an avoidance read over the proven-losing side of the same wallet graph, not the inverted position the plan sketched, because nothing on a bonding curve can be sold before it is bought. Two more of those plays shipped on 2026-09-09. **12.14, the auto-rivalry engine**, is `api/_lib/rivalries.js` plus `GET /api/sniper/rivalries` and the grudge-match strip under the arena leaderboard (`src/play/rivalries.js`, `docs/rivalries.md`): it pairs every trader with the one directly above them and labels the pair an overtake, a debut or a chase. It carries no new table and no cron because the board as it stood at the cutoff is re-derived from close timestamps, which also makes it work retroactively over history already recorded; the copy is assembled from those numbers rather than generated, so a public strip stays instant, free and checkable. **12.12, the embeddable copy-me widget**, is `<trader-card>` (`public/trader-card/element.js`) over a few-kilobyte projection of the profile (`api/_lib/trader-card.js`, `GET /api/sniper/trader-card`), with the snippet and a live preview handed to the leader from an Embed this trader panel on `/trader/:id` and their referral code baked into every link (`docs/trader-card-embed.md`). Verified rendering on a genuinely foreign origin, not just on ours. **12.11 is now half shipped and half a product decision.** The half that was unambiguous shipped: the cross-surface streak counted logins, forges, worlds and walks but not a trade, and a confirmed on-chain buy or sell now keeps it alive (`api/pump/[action].js`). The quest board did not, for two reasons worth recording rather than rediscovering. First, the only per-user actions this platform records are one-time-ish (`pump_agent_trades`, and only for coins launched through the platform; `copy_subscriptions`; `pump_agent_mints`), so a daily board over existing data has exactly one honest daily quest; real daily quests need new event instrumentation on the research surfaces (a signed-in ghost-copy run, a coin's intel opened) and an anti-gaming rule for client-reported events. Second, the obvious quest set the plan sketches ("make 3 trades") rewards trading volume in a real-money product, which is the one incentive this platform should not ship; a board built here should reward research and paper-first habits instead. Both are calls the owner should make before anyone builds it. Still open: 12.6 (agent prediction markets, which the plan itself flags for regulatory framing), the quest board half of 12.11 above, and 12.15 (the first-rug softener, flagged rather than committed). |
