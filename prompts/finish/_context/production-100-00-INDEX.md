...
| [../roadmap/native-widgets.md](../916-roadmap-native-widgets.md) | All four tasks built. The card endpoint, the Android widget and the Windows manifest are live in production (re-verified 2026-09-04, including the Apple hand-off); task 4's WidgetKit extension, Mac app and iOS target are in `apple/` and guarded by `npm run check:apple-widget`. What is left is not code: signing and shipping the two Apple binaries needs row 17, and an in-board Windows check needs a Windows 11 machine. One caveat: 2026-09-04 fixed three defects in the Windows service worker (the Adaptive Card was never fetched, the refresh was never registered, a slot pinned before activation stayed empty) and 2026-09-09 fixed a fourth in the card itself (a dotted array index the board's expression engine refuses, which empties the whole widget), so run that board check only against a deploy carrying both. The worker fix is live; the card fix is not. `npm run check:windows-widget:live` decides that in one command against any origin, and it exits 1 against production as of 2026-09-09 (re-verified 19:44Z against commit `880bdcef8`, revision 00420: 5 findings, all of them the one undeployed card, while the 11 offline checks and the 15 Apple checks pass). That probe is now a registered guard (`data/guards.json`, `docs/guards.md`, `/guards`) rather than a command only this pack mentions. |
