...
| 10 | [10-avatar-likeness-irl-people.md](../007-quality-bar-10-avatar-likeness-irl-people.md) | IRL people: likeness, hands, rig, AR | Partial. Rig half is DONE: animation dignity 10/10 across ten conventions (re-verified 2026-09-08), the clavicle-pivot defect is fixed, `audit:rig-coverage` is 204/204, and the rig-failure degrade path is kill-tested. The likeness audit is now MEASURED rather than open, with 44 rendered views and per-case bounding boxes in `prompts/quality-bar/_generated/10/pass-2026-09-08/` (read its `REPORT.md` first): 1 clean, 2 partial, 3 outright failures. Two causes were found and fixed (avatars were generated on the weaker TRELLIS lane despite the docs promising Hunyuan3D; text-to-3D skipped the rembg cutout so the reference's backdrop was reconstructed as a slab), plus two real production bugs in the vision chain that had left the quality gate fail-open passing everything. **What remains is a re-measure, not a diagnosis:** it needs the deploy plus `CRON_SECRET` for the high tier, neither available in that session. Hunyuan3D still has no matte support at all. |
