...
1. **Embed is free and immediate.** Never gate the embed path behind wallet connect or on-chain deployment. On-chain is a _badge upgrade_, not a requirement.
2. **Anonymous `/app` is the funnel.** Users land, drop a GLB, see it render, _then_ hit the "save this" wall that prompts sign-in. Don't force auth before showing the product.
