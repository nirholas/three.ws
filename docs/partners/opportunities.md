...
| DEXTools, reciprocal 3D embed | Our side is shipped: DEXTools charts are already a provider in every three.ws chart switcher, and as of 2026-09-20 `/coin3d` is framable and keyed by pair address, which is the identifier a DEXTools pair page already holds. The relationship has public receipts in both directions: three $THREE Social Boost wins with DEXTools buybacks behind them | Send [dextools-proposal.md](./dextools-proposal.md) to a DEXTools partnerships or front-end contact. Nothing else is blocked; the embed is live and free whether or not they answer | Owner |
