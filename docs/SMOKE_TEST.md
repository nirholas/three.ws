...
| G. Discover page               | BLOCKED | Requires seeded Base Sepolia wallet. Static analysis: `/public/discover/index.html` + `discover.js` present. `/api/explore.js` endpoint present. Import flow routes to `/hydrate`.                                            |
...
- [ ] **No `DISCOVER_LINK` nav entry wired in dashboard sidebar** — `public/dashboard/index.html` — The community discover page (`/discover`) has no sidebar link in the dashboard navigation. Users who discover an agent via Flow G must navigate directly to the URL. Low-priority UX gap; one-line fix in `public/dashboard/index.html` sidebar nav HTML, but deferred to avoid touching anchor-owned dashboard code during this pass.
