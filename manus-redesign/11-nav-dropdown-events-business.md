...
|---|---|---|---|
| Webinars | Live sessions with the three.ws team | `events/webinars` | `feVideo` |
...
| Hackathons | Build with three.ws alongside the community | `events/hackathons` | `feZap` |
| Past recordings | Replay every session on demand | `events/recordings` | `fePlay` |

## Business items
| Title | Subtitle | route | Icon |
|---|---|---|---|
| Enterprise | Deploy three.ws across your org | `business/enterprise` | `feBriefcase` |
...
| Customer stories | How teams ship faster with three.ws | `business/customers` | `feUsers` |
| Contact sales | Talk to a real human | `business/contact-sales` | `feMessageCircle` |

## What to ship
- Component `chat/src/three.ws/dropdowns/EventsDropdown.svelte` — items above, same template.
- Component `chat/src/three.ws/dropdowns/BusinessDropdown.svelte` — items above, same template.
...
<div class="max-w-[760px] mx-auto px-6 pt-24 text-center">
  <h1 class="three.ws-display">{title}</h1>
