...
- `route` store from task 02. Add a route switch in `App.svelte` if not present:
  ```svelte
  {#if $route === 'pricing'}
    <Pricing />
  ...
  {:else if $route.startsWith('solutions/') || $route.startsWith('events/') || $route.startsWith('business/')}
...
## Acceptance criteria
- Visiting `#pricing` shows this page; the chat is hidden.
- Three cards render side-by-side on desktop, stacked on mobile.
- The Pro card has the inverted (black) styling.
- FAQ accordion expands/collapses.
- Top nav remains visible above the page.

## Out of scope
- Wiring CTAs to any real billing flow — they're presentational.
